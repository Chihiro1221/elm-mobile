...
const beforeUpload: any = async (file: File) => {
  await uploadAvatar(file, auth.profile?.id!)
}
const afterUpload: any = (file: UploadResult) => {
  if (file) {
...
<template>
  <div class="info-container">
    ...
    <div class="binding mt-2">
      ...
      <van-cell class="border" @click="handleBind('请在App中绑定')">
        <template #title>
          <div class="flex items-center h-full">
    ...
    <div class="px-2 py-4">
      <van-button type="danger" color="#c96050" size="small" class="rounded-sm" block @click="logout">
        退出登录
      </van-button>
    </div>
