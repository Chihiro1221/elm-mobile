<script setup lang="ts">
import Navbar from '@/components/common/navbar.vue'
import { handleBind } from '@/hooks/profile'
</script>

<template>
  <div class="recommend-container">
    <Navbar title="推荐有奖" />
    <div class="container">
      <div class="cover">
        <img src="https://elm.cangdu.org/elm/static/img/activity.png" alt="" />
      </div>
      <div class="meta">
        <div class="item" @click="handleBind('请在饿了么App中打开')">
          <img
            src="data:image/png;base64,"
            alt=""
          />
          邀请微信好友
        </div>
        <div class="item" @click="handleBind('请在饿了么App中打开')">
          <img
            src="data:image/png;base64,"
            alt=""
          />
          邀请QQ好友
        </div>
        <div class="item" @click="handleBind('请在饿了么App中打开')">
          <img
            src="data:image/png;base64,"
            alt=""
          />
          面对面邀请
        </div>
      </div>
      <div class="desc">
        <div class="item">
          <div>累计收益</div>
          <div>
            <span>0</span>
            元
          </div>
        </div>
        <div class="item">
          <div>成功邀请</div>
          <div>
            <span>0</span>
            人
          </div>
        </div>
      </div>
      <div class="footer">
        <div>-收益明细-</div>
        <img
          width="28"
          src="data:image/png;base64,"
          alt=""
        />
        <div>还不赶紧去邀请好友</div>
      </div>
    </div>
  </div>
</template>

<style scoped lang="scss">
.recommend-container {
  @apply pt-[46px] bg-[#f5f5f5];
  .container {
    @apply h-screen overflow-hidden;
    .meta {
      @apply flex items-center bg-white py-4;
      .item {
        @apply w-1/3 text-xs text-gray-500 flex flex-col justify-center items-center;
        img {
          @apply w-[60px] h-[60px] mb-2;
        }
      }
    }
    .desc {
      @apply flex items-center py-6;
      .item {
        @apply w-1/2 text-center text-xs text-gray-500;
        span {
          @apply text-base font-bold;
        }
        &:first-child {
          @apply border-r;
          span {
            @apply text-red-500;
          }
        }
      }
    }
    .footer {
      @apply text-center text-xs text-gray-500 flex flex-col justify-center items-center;
      img {
        @apply mt-4 mb-2;
      }
      > div {
        &:last-child {
          @apply text-gray-400;
        }
      }
    }
  }
}
</style>
