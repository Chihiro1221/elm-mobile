...
import {
  ...
  Comment,
  Food,
  MerchantRating, getMerchantRating, MerchantCommentCate, getMerchantCommentCate
} from '@/apis/merchantApi'
import { computed, ref } from 'vue'
import ScrollNav from '@/components/common/scroll-nav.vue'
import Header from './components/header.vue'
import CommentItem from '@/views/merchant/components/comment-item.vue'
...
currentCommentCate.value = commentCate.value[0]
const handleComments = computed(() => {
  const filterArr = comments.value.filter(item => item.rating_text)
  let result = onlyShow.value ? comments.value : filterArr
...
<template>
  <div class='merchant-container'>
    ...
    <van-tabs class='z-[999]' sticky border v-model:active='active' title-active-color='#4d8ee1'
              color='#4d8ee1'>
      <van-tab title='商品'>
      ...
      <van-tab title='评价'>
        <div class='comment-container h-screen bg-[#f3f5f7]'>
          <div class='rating bg-white border-b p-4 flex items-center justify-between h-[100px]'>
            ...
            <div class='line h-full w-[1px] bg-[#e3e5e8]'>
            </div>
            <div class='rating-rate text-xs flex flex-col gap-2'>
              ...
              <div class='flex items-center gap-2'>
                送达时间 <span class='text-gray-500'>{{ rating.deliver_time }}分钟</span>
              </div>
            </div>
          </div>
